...
export default function ShopMegaMenu({ isOpen, onNavigate, onClose }: ShopMegaMenuProps) {
  ...
  const leftMenuItems = [
    { id: 'categories', label: 'Categories', icon: Package, hasChevron: true, description: 'Browse by health benefit' },
    ...
    { id: 'health-issues', label: 'Health Issues', icon: ShieldCheck, hasChevron: true, description: 'Address specific concerns' },
          ...
          <div className="max-w-[1920px] mx-auto px-[40px] py-[48px]">
            <div className="flex gap-[32px]">
              ...
              <div className="flex-shrink-0 w-[280px]">
                ...
                <div className="bg-[#EBF6F4] rounded-[10px] p-[8px] mb-[24px] space-y-[6px]">
                  {leftMenuItems.slice(0, 4).map((item) => {
                    const isActive = item.hasChevron && activeSubmenu === item.id;
                    const hasSubmenu = item.hasChevron && ['categories', 'body-part', 'body-function', 'health-issues'].includes(item.id);
                    ...
                    return (
                      <button
                        ...
                        onMouseEnter={() => {
                          if (hasSubmenu) {
                            setActiveSubmenu(item.id as any);
                          }
                        }}
                        className={`relative w-full px-[16px] py-[14px] text-left rounded-[8px] transition-all duration-200 flex flex-col gap-[0px] group ${
                          isActive 
                            ? 'bg-white shadow-sm' 
                            : 'hover:bg-white/60'
                        }`}
                      >
                        <div className="flex items-start gap-[12px]">
                          <div className={`p-[8px] rounded-[6px] transition-colors ${
                            isActive ? 'bg-[#009296]' : 'bg-white/80 group-hover:bg-[#009296]/10'
                          }`}>
                            <IconComponent className={`w-[18px] h-[18px] ${
                              isActive ? 'text-white' : 'text-[#009296]'
                            }`} />
                          </div>
                          <span className={`flex-1 font-['Inter',sans-serif] font-medium text-[15px] leading-[1.3] ${
                            isActive ? 'text-[#009296]' : 'text-[#003b3c]'
                          }`}>
                            {item.label}
                          </span>
                          {item.hasChevron && (
                            <ChevronRight className={`w-[18px] h-[18px] transition-all duration-200 flex-shrink-0 ${
                              isActive 
                                ? 'text-[#009296] translate-x-1' 
                                : 'text-[#666666] group-hover:translate-x-1'
                            }`} />
                          )}
                        </div>
                        {item.description && (
                          <div className="pl-[46px] pr-[28px]">
                            <p className="font-['Inter',sans-serif] text-[12px] text-[#666666] leading-[1.4]">
                              {item.description}
                            </p>
                          </div>
                        )}
                  ...
                  {leftMenuItems.slice(4).map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-[10px] w-full px-[8px] py-[8px] text-left rounded-[6px] hover:bg-[#F5F5F5] transition-colors duration-200 group"
                      >
                        <IconComponent className="w-[16px] h-[16px] text-[#666666]" />
                        <span className="font-['Inter',sans-serif] text-[14px] text-[#666666] group-hover:text-[#003b3c]">
                          {item.label}
                        </span>
                      </button>
                    );
                ...
                >
                  {column1.map((item) => {
                    ...
                    return (
                      <button
                        key={item.name}
                        onClick={() => onNavigate?.(item.name.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and'))}
                        className="relative flex items-center gap-[12px] w-full text-left font-['Inter',sans-serif] text-[15px] text-[#003b3c] px-[12px] py-[10px] rounded-[8px] transition-all duration-200 group bg-white hover:shadow-[0_2px_8px_rgba(0,0,0,0.08)]"
                      >
              ...
              <div className="flex-shrink-0 w-[360px] flex flex-col gap-[20px]">
                {/* Need Help Choosing? Guidance Card */}
                <div className="bg-gradient-to-br from-[#009296] to-[#007B7F] rounded-[10px] p-[24px] text-white">
                  <div className="flex items-start gap-[12px] mb-[16px]">
                    <div className="p-[10px] rounded-[8px] bg-white/20">
                      <Compass className="w-[24px] h-[24px] text-white" />
                    </div>
                    <div>
                      <h3 className="font-['Inter',sans-serif] text-[17px] mb-[4px]">
                        Not Sure Where to Start?
                      </h3>
                      <p className="font-['Inter',sans-serif] text-[13px] text-white/90 leading-[1.5]">
                        Take our quick assessment and get personalized supplement recommendations based on your health goals.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => onNavigate?.('vitamin-calculator')}
                    className="w-full bg-white text-[#009296] px-[20px] py-[12px] rounded-[8px] font-['Inter',sans-serif] font-medium text-[14px] hover:bg-white/90 transition-colors flex items-center justify-center gap-[8px] group"
                  >
                ...
                {/* Shop Our Specials */}
                <div className="bg-[#EBF6F4] rounded-[10px] p-[24px]">
                  <div className="relative w-full h-[240px] rounded-[8px] overflow-hidden mb-[16px]">
                    <ImageWithFallback
                      src="https://images.unsplash.com/photo-1505751172876-fa1923c5c528?w=400"
                      alt="Shop our specials"
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <h3 className="font-['Inter',sans-serif] text-[17px] text-[#003b3c] mb-[8px]">
                    Shop Our Specials
                  </h3>
                  <p className="font-['Inter',sans-serif] text-[13px] text-[#666666] leading-[1.6] mb-[16px]">
                    Save on trusted formulas, exclusive bundles, and limited-time offers.
                  </p>
                  <button
                    onClick={() => onNavigate?.('specials')}
                    className="flex items-center gap-[8px] font-['Inter',sans-serif] font-medium text-[14px] text-[#009296] hover:gap-[12px] hover:underline transition-all"
                  >
