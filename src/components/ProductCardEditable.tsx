// Editable Product Card Component
...
export default function ProductCardEditable({ 
  ...
  const lastHeadlineValue = useRef<string>('');

  // Set initial content on mount only
  useEffect(() => {
    if (headlineRef.current && !lastHeadlineValue.current) {
      const initialValue = headline || '';
      headlineRef.current.textContent = initialValue;
      lastHeadlineValue.current = initialValue;
    }
  }, []);

  const handleHeadlineChange = (e: React.FormEvent<HTMLDivElement>) => {
    const value = e.currentTarget.textContent || '';
    lastHeadlineValue.current = value;
    onHeadlineChange?.(value);
    
    // Clear content completely if empty to restore placeholder
    if (!value.trim()) {
  ...
  return (
    <div className="basis-0 bg-[#F6F2EC] grow h-full rounded-[10px] flex flex-col group/card">
      <div className="box-border flex flex-col items-center justify-between pb-[10px] md:pb-[20px] pt-[30px] md:pt-[40px] px-[10px] md:px-[20px] hd:px-[20px] h-full">
          ...
          <div className="box-border flex flex-col gap-[10px] md:gap-[20px] items-center px-[10px] md:px-[20px] hd:px-[25px] py-0">
            ...
            <div className="w-full relative group/headline">
              <p
                ...
                data-placeholder="Add product headline using two or three lines"
                className="product-headline font-['STIX_Two_Text:Regular',sans-serif] leading-[1.2] text-[#003b3c] text-center tracking-[-0.4px] xxl:tracking-[-0.44px] hd:tracking-[-0.56px] cursor-text hover:bg-white/40 focus:bg-white/60 px-[8px] py-[4px] rounded-[4px] transition-colors outline-none empty:before:content-[attr(data-placeholder)] empty:before:text-[#003b3c]/40"
              />
            </div>
          </div>
        </div>
        ...
        >
          {hasImage ? (
            <img 
              alt="" 
              className="absolute inset-0 object-cover size-full group-hover/card:scale-105 transition-transform duration-300" 
          ...
          {onQuickView && (
            <button 
              ...
              className="absolute bottom-[0.49px] right-0 hover:opacity-80 transition-opacity cursor-pointer z-10"
            >
